...
schemaUtils.getRules = (fullSchema, required, options) => {
  ...
  if (fullSchema.type === 'string' && fullSchema.pattern !== undefined) {
    const regex = new RegExp(fullSchema.pattern)
    rules.push((val) => (val === undefined || val === null || regex.test(val)) || fullSchema.errorMessage.pattern || options.patternMessage )
  }
  if (fullSchema.type === 'string' && fullSchema.minLength !== undefined) {
    rules.push((val) => (val === undefined || val === null || val.length >= fullSchema.minLength) || fullSchema.errorMessage._ || '')
  }
  if (fullSchema.type === 'string' && fullSchema.maxLength !== undefined) {
    rules.push((val) => (val === undefined || val === null || val.length <= fullSchema.maxLength) || fullSchema.errorMessage._ || '')
  }
  if (['number', 'integer'].includes(fullSchema.type) && fullSchema.maximum !== undefined) {
    rules.push((val) => (val === undefined || val === null || val <= fullSchema.maximum) || fullSchema.errorMessage._ || '')
  }
  if (['number', 'integer'].includes(fullSchema.type) && fullSchema.minimum !== undefined) {
    rules.push((val) => (val === undefined || val === null || val >= fullSchema.minimum) || fullSchema.errorMessage._ || '')
  }
  if (fullSchema.enum) {
