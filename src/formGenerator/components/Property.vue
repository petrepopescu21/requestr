  ...
  >
    ...
    >
      ...
      <v-date-picker v-model="modelWrapper[modelKey]" no-title scrollable>
        <v-spacer />
        <v-btn text class="v-btn--flat" :style="oldFlat" @click="menu = false">Cancel</v-btn>
        <v-btn
          text
          class="v-btn--flat primary--text"
          @click="$refs.menu.save(modelWrapper[modelKey]); change(); input()"
        >OK</v-btn>
      </v-date-picker>
    </v-menu>
      ...
      >
        <template slot="selection" slot-scope="data">
          <div class="v-select__selection v-select__selection--comma">
            ...
            <span
              v-if="![null, undefined].includes(data.item)"
            >{{ data.item + (fullSchema.type === 'array' && data.index !== modelWrapper[modelKey].length - 1 ? ',&nbsp;' : '') }}</span>
          </div>
        </template>
      ...
      >
        <template slot="selection" slot-scope="data">
          <div class="v-select__selection v-select__selection--comma">
            ...
            <span
              v-if="![null, undefined].includes(data.item[itemTitle])"
            >{{ data.item[itemTitle] + (fullSchema.type === 'array' && data.index !== modelWrapper[modelKey].length - 1 ? ',&nbsp;' : '') }}</span>
          </div>
        </template>
        <template slot="item" slot-scope="data">
          <select-icon v-if="itemIcon" :value="data.item[itemIcon]" />
    ...
    >
      <template slot="selection" slot-scope="data">
        ...
        <div
          v-if="![null, undefined].includes(data.item[itemTitle])"
        >{{ data.item[itemTitle] + (fullSchema.type === 'array' && data.index !== modelWrapper[modelKey].length - 1 ? ',&nbsp;' : '') }}</div>
      </template>
      <template slot="item" slot-scope="data">
        <select-icon v-if="itemIcon" :value="data.item[itemIcon]" />
        <select-item :title="data.item[itemTitle]" :options="options" />
    ...
    <div v-else-if="fullSchema.type === 'object'">
      <v-subheader
        ...
        @click="folded = !folded"
      >
        {{ fullSchema.title }}{{typeof modelKey=='number'?`-${modelKey}`:''}}
        &nbsp;
        ...
        <div v-show="!foldable || !folded">
          ...
          <template v-if="fullSchema.oneOf">
            ...
            >
              <tooltip
                slot="append-outer"
                :html-description="oneOfConstProp && oneOfConstProp.htmlDescription"
              />
            </v-select>
            <!--{{ currentOneOf }}-->
            ...
            <v-flex v-for="(itemModel, i) in modelWrapper[modelKey]" :key="i" xs12>
              <v-card class="array-card">
                <v-card-title primary-title class="pa-0">
                  ...
                  <span
                    v-if="itemTitle && modelWrapper[modelKey][i]"
                  >{{ modelWrapper[modelKey][i][itemTitle] }}</span>
                  <v-spacer />
                  ...
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <v-btn
                        ...
                        color="primary"
                        v-on="on"
                        @click="modelWrapper[modelKey].push(JSON.parse(JSON.stringify(modelWrapper[modelKey][i]))); change(); input()"
                      >
...
export default {
  ...
  data() {
    return {
      ...
      fromUrlParams: {},
      loading: false,
      folded: this.options.autoFoldObjects,
  ...
  computed: {
    ...
    foldable() {
      return (
       this.options.foldable && this.parentKey && this.fullSchema.title
      );
    },
    oneOfConstProp() {
...
<style lang="css">
.vjsf-property .vjsf-property .vjsf-property{
  /* float: left; */
}
